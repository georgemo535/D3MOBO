...
var conditionID = "custom";
var applicationID = "custom";
var tutorialFinished = true;
...
// Time variables
var timeFinishDisabled = 0;
var timeFinishTask = 10000000;
var finishTimeMet = true;
...
function refreshResults(){
  var totalLengthData = evaluatedDesigns.length + heuristicDesigns.length;
  var newData = false;

  $.ajax({
    url: "./cgi/check_database_length.py",
    type: "post",
    datatype: "json",
    ...
    success: function(result) {
      ...
      var dataLength = Number(JSON.parse(result.message).data_length);
      newData = (totalLengthData < dataLength);

      var formalDesigns = JSON.parse(result.message).formal_designs;
      var formalObjs = JSON.parse(result.message).formal_objs;
      var pilotDesigns = JSON.parse(result.message).heuristic_designs;
      var pilotObjs = JSON.parse(result.message).heuristic_objs;

      var newFormal = (evaluatedDesigns.length < formalDesigns.length);

      // console.log(formalDesigns);
      // console.log(formalObjs);
      // console.log(pilotDesigns);
      // console.log(pilotObjs);

      if (newData){
        // push new evaluated designs
        var maxFormalDesign = evaluatedDesigns.length;
        for (var i = maxFormalDesign; i < formalDesigns.length; i++){
          var newDesignData = {};
          newDesignData["id"] = i+1;

          var paramValsNorm = normalizeParameters(formalDesigns[i], parameterBounds);
          var objValsNorm = normalizeObjectives(formalObjs[i], objectiveBounds);

          for (var j = 1; j <= numParams; j++){
            newDesignData["x" + j] = parseFloat(paramValsNorm[j-1]).toFixed(2);     
          }

          for (var j = 1; j <= numObjs; j++){
            newDesignData["y" + j] = parseFloat(objValsNorm[j-1]).toFixed(2);
          }

          evaluatedDesigns.push(newDesignData);
        }

        var maxHeuristicDesign = heuristicDesigns.length;
        for (var i = maxHeuristicDesign; i < pilotDesigns.length; i++){
          var newDesignData = {};
          newDesignData["id"] = -i-1;

          var paramValsNorm = normalizeParameters(pilotDesigns[i], parameterBounds);
          var objValsNorm = normalizeObjectives(pilotObjs[i], objectiveBounds);
    ...
    error: function(result){
        console.log("Error in refreshing: " + result.message);
    }
  });
}

async function renderMoboInterfaceDesigner() {
  ...
  document.getElementById('evaluation-button').style.visibility = 'hidden';
  ...
  parent.task.document.getElementById("send-parameters-button").addEventListener("click", sendEvalParams);
...
function sendEvalParams(){
  ...
  var ipAddress = parent.task.document.getElementById("ip-address").value;
  var portValue = parent.task.document.getElementById("port-id").value;

  var queryUrl = "http://" + ipAddress + ":" + portValue
